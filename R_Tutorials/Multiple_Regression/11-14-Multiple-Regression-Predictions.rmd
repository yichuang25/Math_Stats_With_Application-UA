---
title: "Inferences using Multiple Regression in R"
output:
...
# Load data.
x <- 0:4
Y <- c(-1.28, 0.16, -2.35, -12.52, -44.35)
n <- length(x)
# Create X.
...
The following code creates a function for calculating the endpoints for the  confidence intervals for a given $x^*$. The function takes as input $\boldsymbol{\hat\beta}$ as argument *B*, the matrix $\mathbf{(X^TX)^{-1}}$ as argument *C*, sample standard deviation $S$ (as argument *S*), number of observations $n$ (as argument *n*), $\alpha$ (as argument *alpha*), and value $x$ (as argument *x*).
```{r CIFunction}
