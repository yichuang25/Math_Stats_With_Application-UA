...
Again we the following observations $Y_1, Y_2, \dots, Y_n$ following linear model $E(Y) = \beta_0 + \beta_1 x$: 
...
There are several equivalent processes for solving the linear system $(\mathbf{X'} \mathbf{X}) \boldsymbol{\hat\beta} = \mathbf{X'} \mathbf{Y}$ and, thus, calculating the least squares estimators.
...
| $x$ | 0 | 1 | 2 | 3 | 4 | 
|:---:|:---:|:---:|:---:|:---:|:---:|
|$y$ | -1.28| 0.16 | -2.35 | -12.52 | -44.35 |

```{r plotNonlinear, echo = FALSE}
plot(x = 0:4, xlab = "x",
     y = c(-1.28, 0.16, -2.35, -12.52, -44.35), ylab = "Y",
     xlim = c(-1/4, 4 + 1/4), 
...
# Load data.
x2 <- 0:4
y2 <- c(-1.28, 0.16, -2.35, -12.52, -44.35)
n2 <- length(x2)
# Create X.
